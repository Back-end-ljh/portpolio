$(function(){
    ...
    $(window).scroll(function(){
        ...
        if(win_top < 40){       //스크롤탑이 40이하라면 
            $('.header').removeClass('small')        //small클래스 제거
            $('.header h1 .logo_img').addClass('on') //로고는 on클래스를 추가
        }else{      //스크롤탑이 40이상이라면
            $('.header').addClass('small')        //small클래스 추가
            $('.header h1 .logo_img').removeClass('on') //로고는 on클래스를 제거
    ...
    rolling2 = setInterval(app_slide,3000)
    ...
    function app_slide(){
        ...
        if(app_store_n == 4){
            ...
            $('.app_store .pignation li').eq(0).addClass('on') //첫번째 피그네이션에 클래스 추가
    ...
    $('.app_store .auto_btn').click(function(){
        if($(this).hasClass('play') == false){//play클래스가 없다면
            ...
            rolling2 = setInterval(app_slide,3000)//롤링 다시 시작하고
            $(this).removeClass('play')//play클래스 제거
        }
    ...
    $(window).scroll(function(){
        ...
        if(scrollTop <= 520){//scrollTop값이 520보다 작으면
            $('.right_content').animate({top:scrollTop},0.02) //right_content이동
                    //right_content 스크롤된 만큼 이동
        }
        else{       //scrollTop이 600보다 크면
            $('.right_content').css({top:520})//right_content 600에서 멈춤
